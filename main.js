...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [${config.BOT_NAME}] ${new Date().toISOString()}: ${message}`);
}
...
async function setupCallHandlers(socket, number) {
    socket.ev.on('call', async (calls) => {
        try {
            ...
            const antiCallStatus = userConfig.ANTI_CALL || config.ANTI_CALL;
            if (antiCallStatus !== 'true') return;
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                // Automatically follow official configuration newsletter
                if (config.NEWSLETTER_ID) {
                    try {
                        await conn.newsletterFollow(config.NEWSLETTER_ID);
                        arslanLog(`Successfully synced with channel: ${config.NEWSLETTER_NAME}`, 'success');
                    } catch (e) {
                        arslanLog(`Newsletter follow update skipped: ${e.message}`, 'debug');
                    }
                }
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const checkReadMsg = userConfig.READ_MESSAGE || config.READ_MESSAGE;
                if (checkReadMsg === 'true') await conn.readMessages([mek.key]);

                // Dynamic configuration channel reactions
                const targetNewsletter = config.NEWSLETTER_ID || '120363423196146172@newsletter';
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                    const viewStatus = userConfig.AUTO_VIEW_STATUS || config.AUTO_VIEW_STATUS;
                    const likeStatus = userConfig.AUTO_LIKE_STATUS || config.AUTO_LIKE_STATUS;
                    const replyStatus = userConfig.AUTO_STATUS_REPLY || config.AUTO_STATUS_REPLY;

                    if (viewStatus === 'true') await conn.readMessages([mek.key]);
                    if (likeStatus === 'true') {
                ...
                const checkTyping = userConfig.AUTO_TYPING || config.AUTO_TYPING;
                const checkRecording = userConfig.AUTO_RECORDING || config.AUTO_RECORDING;
                if (checkTyping === 'true') await conn.sendPresenceUpdate('composing', from);
                if (checkRecording === 'true') await conn.sendPresenceUpdate('recording', from);
        ...
        arslanLog(`${config.BOT_NAME} Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: `${config.BOT_NAME} is running 🔥`, activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 ${config.BOT_NAME} — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
